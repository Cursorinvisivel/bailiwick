...
2. **Capture is enforced; promotion is human-gated.** Under **Claude Code** and **Codex CLI**
   (>= 0.147, once its hooks are trusted), harness hooks stage raw transcripts of substantial
   sessions *automatically* (Gemini/Copilot capture is still a manual step — §10, §14); a
   human-gated `/curate` step distils them into the library. Nothing enters the
...
Guarantees (precise): capture is automatic for **Claude Code** and **Codex** sessions where the
hooks run — for Codex that means codex-cli >= 0.147 *and* its one-time hook trust granted, since an
untrusted hook does not fire (Gemini/Copilot need the manual flow); raw captures are **never added to the target repo** by the framework
...
| **Codex** (global-layer + marker) | global `~/.codex/AGENTS.md` layer + `.bailiwick.local.md` marker; native workflow skills in `~/.codex/skills/` | `~/.codex/AGENTS.md` (loaded first → repo `AGENTS.md` keeps precedence) | **`~/.codex/config.toml`** for active MCP; `.codex/config.toml` is a repo-local draft/reference generated by `--with-agents` | **Automatic** — `Stop`/`SessionEnd` hooks (codex-cli >= 0.147) run the same `capture_session.py` + `capture_backup.sh` pair as Claude Code; inert until each hook is trusted once | **Enforced** — `PreToolUse` hook wired to the guardrail (**deny** + `BAILIWICK_BREAK_GLASS=1` allow-once; one-time trust prompted on first fire in the `codex` CLI, persisted in config) |
...
- **Capture can't be forgotten — under Claude Code and Codex.** There, session capture is a harness
  hook, not a discipline; knowledge is never lost even when you skip curation. Under Codex the hook
  must be trusted once before it fires (§10). Gemini and Copilot use **manual** capture until their
  lifecycle-capture adapters are wired (§10) — the guarantee is not universal.
...
- **Adapter completeness varies.** Full under Claude Code; Codex has the guardrail and automatic
  capture but no injected session context (its knowledge map is the static global `AGENTS.md` layer);
  Gemini gets the guardrail with manual capture; Copilot is policy-only and local-VS-Code-only (§10).
  Codex hooks need a one-time trust, granted per hook definition in the `codex` CLI.
