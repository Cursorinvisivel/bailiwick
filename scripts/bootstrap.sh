...
Options:
  ...
  --install-tools  Install the once-per-machine prerequisites when missing: the
                   ...
                   the global Claude skill symlinks (~/.claude/skills/: /curate, /enrich, /learn, /metrics, /investigate, /purge, /sign), the
...
rm_symlinks() {  # only symlinks that resolve INTO this clone
  local d link tgt
  for d in "$HOME/.claude/skills" "$HOME/.claude/agents" "${CODEX_HOME:-$HOME/.codex}/skills"; do
    [ -d "$d" ] || continue
    for link in "$d"/*; do
      [ -L "$link" ] || continue
...
rm_stage_adapters() {  # only generated files carrying the bailiwick marker (ADR-010 Amendment 1)
  local d f
  for d in "$HOME/.gemini/agents" "${CODEX_HOME:-$HOME/.codex}/agents" "$HOME/.copilot/agents"; do
    [ -d "$d" ] || continue
    for f in "$d"/bailiwick-*; do
      [ -f "$f" ] || continue
      grep -q "GENERATED by bailiwick" "$f" 2>/dev/null || continue
      if DRY; then plan "remove generated stage adapter $f"
      else rm -f "$f" && echo "  removed: generated stage adapter $f"; fi
    done
  done
}
...
if [ "$DRY_RUN" -eq 1 ] && [ "$INSTALL_TOOLS" -eq 1 ]; then
  ...
  echo "    • symlink Quality Workflow stages into ~/.claude/agents/ (native subagents, ADR-010)"
...
# shadow mode stays zero-footprint; activation is this user-scope install.
AGENTS_SRC_DIR="$BAILIWICK_ROOT/agents"
_stage_missing=0; _stage_total=0
for _a in "$AGENTS_SRC_DIR"/*.md; do
  [ -f "$_a" ] || continue
  _aname="$(awk 'NR==1{if($0!="---")exit} /^---$/&&NR>1{exit} /^name:[[:space:]]*/{sub(/^name:[[:space:]]*/,"");print;exit}' "$_a")"
  [ -n "$_aname" ] || continue
  _link="${HOME}/.claude/agents/${_aname}.md"
  _stage_total=$((_stage_total+1))
  if [ ! -e "$_link" ] && [ "$INSTALL_TOOLS" -eq 1 ]; then
    echo "  --install-tools: linking ${_aname} stage as a native subagent (~/.claude/agents/${_aname}.md)…"
    mkdir -p "$(dirname "$_link")"; ln -s "$_a" "$_link" 2>/dev/null || true
  fi
  [ -e "$_link" ] || _stage_missing=$((_stage_missing+1))
done
if [ "$_stage_missing" -eq 0 ] && [ "$_stage_total" -gt 0 ]; then
  AGENT_STAGE_STATUS="✓ ${_stage_total} Quality Workflow stage(s) linked as native subagents (~/.claude/agents/: bailiwick-implement, bailiwick-quality, …)"
else
  AGENT_STAGE_STATUS="✗ ${_stage_missing}/${_stage_total} stages not linked as subagents — re-run with --install-tools, or symlink each frontmattered \$BAILIWICK/agents/*.md into ~/.claude/agents/<name>.md"
fi
...
# (~/.copilot/agents/<name>.agent.md), produced from the canonical frontmattered agents/*.md.
# The Claude-specific `tools:` field is dropped (each tool's defaults apply); a canonical toolset
# without Edit/Write maps to Codex `sandbox_mode = "read-only"`. Regenerated on every
# --install-tools run; a same-name file WITHOUT the GENERATED marker is never overwritten.
if [ "$INSTALL_TOOLS" -eq 1 ]; then
  if command -v python3 >/dev/null 2>&1; then
    BAILIWICK_ROOT="$BAILIWICK_ROOT" CODEX_HOME="${CODEX_HOME:-}" python3 <<'PY'
import os, re
root = os.environ["BAILIWICK_ROOT"]
home = os.path.expanduser("~")
codex_home = os.environ.get("CODEX_HOME") or os.path.join(home, ".codex")
MARK = "GENERATED by bailiwick"
gen = skip = 0
for fn in sorted(os.listdir(os.path.join(root, "agents"))):
    if not fn.endswith(".md"):
        continue
    text = open(os.path.join(root, "agents", fn), encoding="utf-8").read()
    if not text.startswith("---\n") or "\n---\n" not in text[4:]:
        continue
    end = text.index("\n---\n", 4)
    fm, body = text[4:end], text[end + 5:]
    fields = dict(re.findall(r"^(name|description|tools):[ \t]*(.+)$", fm, re.M))
    name, desc = fields.get("name"), fields.get("description")
    if not name or not desc:
        continue
    tools = fields.get("tools", "")
    read_only = bool(tools) and not re.search(r"\b(Edit|Write)\b", tools)
    note = "%s from $BAILIWICK/agents/%s -- edit the source and re-run --install-tools; do not edit here." % (MARK, fn)
    md_adapter = "---\n# %s\nname: %s\ndescription: %s\n---\n%s" % (note, name, desc, body)
    toml_desc = desc.replace("\\", "\\\\").replace('"', '\\"')
    toml_adapter = ("# %s\n" % note
                    + 'name = "%s"\n' % name
                    + 'description = "%s"\n' % toml_desc
    ...
    outs = [
        (os.path.join(home, ".gemini", "agents", name + ".md"), md_adapter),
        (os.path.join(home, ".copilot", "agents", name + ".agent.md"), md_adapter),
        (os.path.join(codex_home, "agents", name + ".toml"), toml_adapter),
    ]
    for dest, content in outs:
        ...
        if os.path.exists(dest):
            try:
                ...
                old = ""
            if MARK not in old:
                print("  SKIP (unmanaged file exists, not overwriting): %s" % dest)
                skip += 1
                continue
            if old == content:
                continue
        ...
        gen += 1
print("  --install-tools: multi-tool stage adapters — generated/updated %d, skipped %d" % (gen, skip))
PY
  else
    echo "  --install-tools: python3 missing — skipping Gemini/Codex/Copilot stage adapters"
  fi
fi
_mt=0
for _d in "$HOME/.gemini/agents" "${CODEX_HOME:-$HOME/.codex}/agents" "$HOME/.copilot/agents"; do
  [ -d "$_d" ] || continue
  _mt=$(( _mt + $(find "$_d" -maxdepth 1 -name 'bailiwick-*' 2>/dev/null | wc -l) ))
done
_mt_expect=$(( _stage_total * 3 ))
if [ "$_mt" -ge "$_mt_expect" ] && [ "$_stage_total" -gt 0 ]; then
  MT_AGENT_STATUS="✓ ${_mt} multi-tool stage adapter(s) generated (Gemini ~/.gemini/agents/, Codex ~/.codex/agents/, Copilot ~/.copilot/agents/)"
elif [ "$_mt" -gt 0 ]; then
  MT_AGENT_STATUS="⚠ ${_mt}/${_mt_expect} multi-tool stage adapters present — re-run with --install-tools to regenerate"
else
  MT_AGENT_STATUS="✗ multi-tool stage adapters (Gemini/Codex/Copilot) not generated — re-run with --install-tools (needs python3)"
fi
